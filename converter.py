...
4. Converted files will appear Excel2CSV_Output in the working directory.
    ...
    for dir, subdir, files in os.walk(excel_dump):
        for file in files:
            ...
            #XLRDErrors may be encountered if file being processed is not true .xlsx file --> this may occur if file has .xlsx exentsion but is not a true excel file
                ...
                print("Failed to convert "+file+" - "+str(unicode_error))
                continue
            except OSError as os_error:
                print("Failed to convert "+file+" - "+str(os_error))
                continue

            #removes original .xlsx file from output folder --> output folder contains unique csv files unless file could not be converted
            os.remove(file)
